...
plot_dir = fullfile(vol,'results/plot');
...
%% Loop through alternatives - new model with Mentalizing
alphas = [.1:.1:.9];
betas = [.9:-.1:.1];
...
for npar = 1:length(alphas)
    ...
    for npl = 1:2
        ...
        if do_save
            if npl==2; beta=0; end
            print(fullfile(plot_dir,['prob_acc_alt_' plotlabel{npl} '_alpha' num2str(alpha) '_beta' num2str(beta) '.png']),'-dpng','-r600');
        end
    end
end
